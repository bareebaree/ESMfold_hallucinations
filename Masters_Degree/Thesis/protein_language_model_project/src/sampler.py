# -*- coding: utf-8 -*-
"""
Created on Tue May 13 22:49:01 2025
...
class DirectedEvolution:
    ...
    def __call__(self, iteration_number: int = 0) -> Tuple[List[str], np.ndarray]:
        ...
        pos_mask = pos_mask.bool().reshape(self.parallel_chains, -1)
        ...
        for i in range(self.n_steps):
            ...
            with torch.no_grad():
                for step in range(max_u):
                    ...
                    if self.max_mutations > 0:
                        ...
                        mask_flag = (dist == self.max_mutations).bool().reshape(self.parallel_chains)
                        mask = utils.mutation_mask(cur_chains_oh, self.wt_oh).reshape(self.parallel_chains, -1)
            ...
            grad_y = self._compute_gradients(ohs, proposed_PoE).detach()
            ...
            with torch.no_grad():
                ...
                for id in range(len(onehot_Idx)):
                    ...
                    log_ratio += u_mask[:, id] * (
                        cd_reverse.log_prob(onehot_Idx[id]) - forward_categoricals[id].log_prob(onehot_Idx[id])
                    )
            ...
            if self.verbose:
                ...
                for idx, variant in enumerate(x_strs):
                    print(f'>step {i}, chain {idx}, acceptance rate: {log_acc[idx].exp().item():.4f}, '
                          f'product of experts score: {PoE[idx]:.4f}')
        ...
        # === Logging after all steps ===
        if self.verbose:
            timestamp = int(time.time())
    
            # === Master CSV appending all iterations ===
            master_log_path = "/mnt/c/Users/james/Masters_Degree/Thesis/protein_language_model_project/results/evoprotgrad_run_log_master.csv"
            os.makedirs(os.path.dirname(master_log_path), exist_ok=True)
            master_exists = os.path.isfile(master_log_path)
            
            with open(master_log_path, mode='a', newline='') as master_file:
                writer = csv.writer(master_file)
                if not master_exists:
                    writer.writerow(['pdb_id', 'iteration', 'step', 'chain_idx', 'sequence', 'score'])
            
                for step_idx, (chains_oh, scores) in enumerate(zip(self.chains_oh_history, self.PoE_history)):
                    decoded_seqs = self.canonical_chain_tokenizer.decode(chains_oh)
                    for chain_idx, seq in enumerate(decoded_seqs):
                        writer.writerow([
                            self.pdb_id,
                            iteration_number,
                            step_idx,
                            chain_idx,
                            ''.join(seq.split()),
                            f'{scores[chain_idx].item():.5f}'
                        ])

    
            print(f"📄 Appended all steps for iteration {iteration_number} to master log")
    
            # === Per-iteration log (for EvoProtGrad compatibility) ===
            iter_log_filename = f"evoprotgrad_run_log_iter{iteration_number}_{timestamp}.csv"
            iter_log_path = os.path.join("/mnt/c/Users/james/Masters_Degree/Thesis/protein_language_model_project/results", iter_log_filename)
            
            with open(iter_log_path, mode='w', newline='') as iter_file:
                writer = csv.writer(iter_file)
                writer.writerow(['pdb_id', 'iteration', 'step', 'chain_idx', 'sequence', 'score'])  # optional header
                for step_idx, (chains_oh, scores) in enumerate(zip(self.chains_oh_history, self.PoE_history)):
                    decoded_seqs = self.canonical_chain_tokenizer.decode(chains_oh)
                    for chain_idx, seq in enumerate(decoded_seqs):
                        writer.writerow([
            ...
            print(f"📄 Wrote per-iteration log to {iter_log_filename}")
